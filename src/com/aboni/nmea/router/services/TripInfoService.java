...
public class TripInfoService extends JSONWebService {
	...
	private static final String sql = "select sum(dist), max(speed), min(TS), max(TS), max(maxSpeed) from track where tripid=?";
	private static final double SPEED_THRESHOLD = 0.3;
	private static final String sql1 = "select sum(dTime) from track where tripid=? and anchor=0 and speed>" + SPEED_THRESHOLD;
