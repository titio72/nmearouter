...
public class TSAGeoMag {
    ...
    private void readWMM() {
        try (InputStream isInput = new FileInputStream(new File(Constants.WMM))) {
            //open data file and parse values
            Reader is = new InputStreamReader(isInput);
            ...
            logger.log(Level.FINEST, "TSAGeoMag Epoch is: {0}", epoch);
            logger.log(Level.FINEST, "TSAGeoMag default date is: {0}", defaultDate);
            str.nextToken();
            str.nextToken();
    ...
    private void calcGeoMag(double fLat, double fLon, double year, double altitude) {
        ...
        logger.log(Level.FINER, "Dec is: {0}", dec);
    ...
    public double getDeclination(double dlat, double dlong) {
        calcGeoMag(dlat, dlong, defaultDate, DEFAULT_ALTITUDE);
        return dec;
    }
