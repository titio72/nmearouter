...
public class MsgNavDataImpl implements MsgNavData {
    ...
    public JSONObject toJSON() {
        ...
        if (getWaypoint()!=null) {
            ...
            waypoint.append("latitude", Utils.formatLatitude(getWaypoint().getLatitude()));
            waypoint.append("longitude", Utils.formatLongitude(getWaypoint().getLongitude()));
            waypoint.append("dec_latitude", getWaypoint().getLatitude());
            waypoint.append("dec_longitude", getWaypoint().getLongitude());
            if (getDestinationWaypointNo()!=0xFF) waypoint.append("no", getDestinationWaypointNo());
            res.append("waypoint", waypoint);
        }

        if (!Double.isNaN(getBTW())) res.append("btw", getBTW());
        if (!Double.isNaN(getDTW())) res.append("btw", getDTW());
        if (!Double.isNaN(getBearingFromOriginToDestination())) res.append("bearingOriginDest", getBTW());
        if (getOriginWaypointNo()!=0xFF) res.append("originWptNo", getBTW());
        res.append("arrived", isArrived());
        res.append("perpendicular_crossed", isPerpendicularCrossed());
        res.append("eta_utc", getETA().toString());
        res.append("eta", getETA().atZone(ZoneId.systemDefault()).toString());
